...
function signJWT(payload, secret) {
  ...
  const sig = b64url(crypto.createHmac("sha256", secret).update(message).digest());
  return `${message}.${sig}`;
}
...
function generateSecrets(envContent) {
  ...
  set(
    "SUPABASE_ANON_KEY",
    signJWT({ iss: "supabase", role: "anon", iat: now, exp: expiry }, jwtSecret)
  );
  set(
    "SUPABASE_SERVICE_ROLE_KEY",
    signJWT({ iss: "supabase", role: "service_role", iat: now, exp: expiry }, jwtSecret)
  );
...
function main() {
  ...
  if (env.LANGFUSE_PUBLIC_KEY?.includes("CHANGE_ME")) {
    ...
    console.log(`     2. Open ${C}http://localhost:3001${R} → create an org + project`);
